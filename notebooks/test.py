...
def load_and_prepare_data(sku_id):
    ...
    df_merged = pd.merge(
        df,
        geo_df[["Day", "sku_id", "customer_city", "customer_postal_code", "customer_type", "category"]],
        on=["Day", "sku_id"],
        how="left"
    )
...
def load_category_data():
    """
    Load minimal data required for category-level demand plot
    ...
    if "category" not in df.columns:
        print("⚠️ 'category' column not found. Category plot skipped.")
        return None
...
def plot_category_demand_bar(df):
    ...
    plt.title("Total Demand by Category", fontsize=15, fontweight="bold")
    plt.xticks(rotation=45, ha="right")
...
def main():
    ...
    # ==========================================================
    # CATEGORY-LEVEL EDA (SAFE, ONE-TIME)
    # ==========================================================
    print("\n[EDA] Category-level demand analysis...")
    category_df = load_category_data()
    if category_df is not None:
        plot_category_demand_bar(category_df)
